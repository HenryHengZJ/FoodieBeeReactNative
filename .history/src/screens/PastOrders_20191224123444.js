...
class PastOrders extends Component {
    ...
    fetchData = () => {
        ...
        var headers = {
            ...
            'Authorization': 'Bearer ' + this.props.getJWTToken.jwttoken,
        }
        ...
        .then((response) => {
            if (response.status === 401) {
                getNewToken.getNewToken(this.props.getJWTToken.jwttoken, this.props.getRefreshToken.refreshtoken, this.props.setJW, (err, result) => {
                    if (err) {
                        this.setState({
                            loading: false,
                            empty: true,
                ...
                this.filterPastOrder(data)
                if (response.headers['x-auth'].split(" ")[0] !== 'undefined') {
                    deviceStorage.saveItem("jwttoken", response.headers['x-auth'].split(" ")[0]);
                    this.props.setJWT(response.headers['x-auth'].split(" ")[0])
                }
               
                if (response.headers['x-auth'].split(" ")[1] !== 'undefined') {
                    deviceStorage.saveItem("refreshtoken", response.headers['x-auth'].split(" ")[1]);
                    this.props.setRefreshToken(response.headers['x-auth'].split(" ")[1])
                }
            }
        })
        .catch(err => {
