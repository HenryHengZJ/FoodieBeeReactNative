...
class OrderDetail extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
            ...
            firstline_notification: "", 
            secondline_notification: "",
            notification_type: "",
    ...
    openMaps = (lat, lng) => {
        ...
        this.watchID = navigator.geolocation.getCurrentPosition((position) => {
            ...
            }).catch(err =>  alert('Unable to open maps')); 
          },
          (err) => {
    ...
    postReview = (reviewComment, rating) => {
        ...
        const { selectedReview, selectedLunchOrder } = this.state;
        ...
        var body = {
          ...
          catererID: selectedLunchOrder.catererDetails[0]._id,
        ...
        if (Object.keys(selectedReview).length === 0) {
            var url = apis.POSTreview

            axios({method: 'post', url: url, data: body, headers: headers})
            .then((response) => {
                if (response.headers['x-auth'] !== 'undefined') {
                    deviceStorage.saveItem("jwttoken", response.headers['x-auth']);
                    this.props.setJWT(response.headers['x-auth'])
    ...
    getLunchOrder = () => {
        ...
        var url = apis.GETlunchorder + "?_id=" + selectedLunchOrder._id;
        ...
        .then((response) => {
            if (response.status === 200) {
                ...
                if (response.data.length > 0) {
                    this.setState({
                        selectedLunchOrder: response.data[0],
                        loadingVisible: false,
                    })
                }
                else {
    ...
    changePickUpTime = () => {
        ...
        const { selectedLunchOrder, selectedPickUpTime } = this.state;
        ...
        var url = apis.PUTupdatelunchorder + "?_id=" + selectedLunchOrder._id;
        ...
        .then((response) => {
            ...
            if (response.status === 201) {
                this.setState({
                    showNotification: true,
                    firstline_notification: "Updated pickup time", 
                    secondline_notification: "Pickup meal at the updated time",
    ...
    cancelOrder = () => {
        ...
        .then((response) => {
            ...
            if (response.status === 201) {
                this.setState({
                    showNotification: true,
                    firstline_notification: "Order cancelled", 
                    secondline_notification: "You have cancelled order #" + selectedLunchOrder.orderNumber,
                    notification_type: "Success",
                }, () => {
                    this.getLunchOrder();
                })
    ...
    cancelPaymentIntent = () => {
        ...
        var url = apis.DELETE_cancel_payment_intent + "?paymentIntentID=" + selectedLunchOrder.paymentIntentID;
    ...
    mealPickedUp = () => {
        ...
        .then((response) => {
            if (response.status === 201) {
                this.setState({
                    showNotification: true,
                    firstline_notification: "Meal picked up", 
                    secondline_notification: "You can now leave your reviews",
                    notification_type: "Success",
                }, () => {
                    this.getLunchOrder();
                })
            ...
            <ScrollView         
                ...
                <View style={{marginBottom: 150}}>
                    ...
                    <View style={{ marginTop:20, marginLeft:20, marginRight:20, flexDirection: 'column'}}>
                        ...
                        <Text style={{ flex: 1, color: '#FF5722', fontWeight: '600', marginTop:10, fontSize: 18}}>
                            Order #{selectedLunchOrder.orderNumber}
                        </Text>

                    </View>

                    <View style={{ marginTop:0, marginLeft:20, marginRight:20, flexDirection: 'column'}}>
                    ...
                    <View style={{marginLeft: 20, marginRight: 20, marginTop:20, flexDirection: 'row'}}>
                        {   selectedLunchOrder.markitem.map((data, index) => {
                                return(
                                    <View style={{flexDirection: 'row', marginRight: 20}}>
                                       
                                        <Text style={{ opacity: 0.7, marginLeft: 5, fontSize: 15, alignSelf:'center', alignItems:'center', justifyContent:'center', fontWeight: '500', color: colors.greyBlack  }}>
                                            {data}
                                        </Text>
                                    </View>
                                )
                     ...
                     <View style={{marginLeft: 20, marginRight: 20, marginTop:20, flexDirection: 'row'}}>
                        ...
                        <Text style={{marginLeft: 5, color: colors.greyBlack, fontWeight: '500', fontSize: 15, alignSelf: 'flex-end'}}>
                            {selectedLunchOrder.orderStatus === "pending" ? "charge pending" : "charged"}
                        </Text>
                    ...
                    {selectedLunchOrder.orderStatus === "pickedup" || selectedLunchOrder.orderStatus === "accepted" || selectedLunchOrder.orderStatus === "rejected" ?
                        ...
                        <View style={{ padding:15, flexDirection: 'column' }}>
                            ...
                            <Text style={{ alignSelf: 'center', textAlign: 'center', paddingHorizontal: 20, lineHeight:25, color: 'gray', fontWeight: '400', fontSize: 15}}>
                                {selectedLunchOrder.orderStatus === "pickedup" ? 
          ...
          <View 
              ...
              <View style={{ padding:15, flexDirection: 'row' }}>
                ...
                <TouchableOpacity
                    ...
                    onPress={this.state.selectedPickUpTime === "" ? this.getLunchOrder.bind(this) : this.changePickUpTime.bind(this)}
                    >
              ...
              <View 
                  ...
                  <TouchableOpacity
                    ...
                    onPress={this.cancelPaymentIntent.bind(this)}
                    >
                        <Text style={{ alignSelf: 'center', color: 'white', fontWeight: '500', fontSize: 17}}>
                              Cancel Order
                        </Text>
          ...
          <View 
              ...
              <View 
                  ...
                  <TouchableOpacity
                    ...
                    onPress={this.mealPickedUp.bind(this)}
                    >
                        <Text style={{ alignSelf: 'center', color: 'white', fontWeight: '500', fontSize: 17}}>
                            Meal Picked Up
                        </Text>
    ...
    render() {
        ...
        const { error, internetStatus, loadingVisible, selectedLunchOrder, showNotification, notification_type, firstline_notification, secondline_notification} = this.state
        ...
        const notificationMarginTop = showNotification ? 10 : 0;
            ...
            return (
                ...
                <ReviewModal
                    ...
                    isPostButtonActive={Object.keys(this.state.selectedReview).length === 0 ? false : true}
                    postReview={(reviewComment, rating) => this.postReview(reviewComment, rating)}
                    handleClosePress={() => this.setState({reviewModal: false})}
                    profilesrc={Object.keys(this.state.selectedReview).length === 0 ? this.state.selectedLunchOrder.catererDetails[0].profilesrc : this.state.selectedReview.catererDetails[0].profilesrc}
                    catererName={Object.keys(this.state.selectedReview).length === 0 ? this.state.selectedLunchOrder.catererDetails[0].catererName : this.state.selectedReview.catererDetails[0].catererName}
                    starCount={Object.keys(this.state.selectedReview).length === 0 ? 0 : this.state.selectedReview.customerRating}
                    reviewComment={Object.keys(this.state.selectedReview).length === 0 ? "" : this.state.selectedReview.customerComment}>
                </ReviewModal> : null }
                ...
                <View style={[styles.notificationWrapper, { marginTop: notificationMarginTop }]}>
                    <Notification
                        ...
                        firstLine = {firstline_notification !== "" ? firstline_notification : "Unfortunately, some errors occured."}
                        secondLine = {secondline_notification !== "" ? secondline_notification : "Please try again."}
                    />
                </View>
