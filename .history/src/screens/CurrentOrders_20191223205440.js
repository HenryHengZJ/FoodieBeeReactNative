...
class CurrentOrders extends Component {
    ...
    fetchData = () => {
        ...
        .then((response) => {
            ...
            alert(typeof response.headers['x-auth'].split(" ")[0])
            if (typeof response.headers['x-auth'].split(" ")[0] !== 'undefined') {
              //  alert('set JWT = ' + response.headers['x-auth'].split(" ")[0])
                deviceStorage.saveItem("jwttoken", response.headers['x-auth'].split(" ")[0]);
                this.props.setJWT(response.headers['x-auth'].split(" ")[0])
            }
           
            if (typeof response.headers['x-auth'].split(" ")[1] !== 'undefined') {
