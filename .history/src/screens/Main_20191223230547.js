...
class Main extends Component {
  ...
  _loadInitialState = async (setJWT, setRefreshToken, setUID) => {
    try {
      const useridvalue = await AsyncStorage.getItem('userid');
      const jwt = await AsyncStorage.getItem('jwttoken');
      const refreshtoken = await AsyncStorage.getItem('refreshtoken');
      if (jwt!== null && refreshtoken !== null  && useridvalue !== null ) {
        setUID(useridvalue);
        setJWT(jwt)
  ...
  componentWillMount() {
    ...
    const {setJWT, setUID, setRefreshToken} = this.props;

    this._loadInitialState(setJWT, setRefreshToken, setUID).done();
